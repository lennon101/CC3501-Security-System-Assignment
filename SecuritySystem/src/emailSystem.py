...
msg.attach(part)
part.set_payload(open(picname2, 'rb').read())
Encoders.encode_base64(part)
part.add_header('Content-Disposition','attachment; filename="%s"' % os.path.basename(picname2))
msg.attach(part)
part.set_payload(open(picname3, 'rb').read())
Encoders.encode_base64(part)
part.add_header('Content-Disposition','attachment; filename="%s"' % os.path.basename(picname3))
msg.attach(part)
